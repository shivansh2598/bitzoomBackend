...
app.post('/upload',verifytoken1,(req,res,next)=>{
	// console.log(req)
	try{
	...
	if(req.files===null || req.body.subject==='' || req.body.semester==='' || req.body.branch==='')
	{
		res.json({status:422,msg:'Missing Required Parameters'})
	}
	else if(req.files.file.mimetype==='image/jpeg'||req.files.file.mimetype==='image/png'||req.files.file.mimetype==='application/pdf')
	next();
	else
	{
	...
	imageFile.mv(`${__dirname}/public/${req.files.file.name}`, err => {
		...
		else
		{
			...
			// console.log(req.user.uploads)
					User1.updateOne({_id:req.userId},{$inc : {uploads:1}},(err,reslt)=>{
						 if(err)
						 {
							res.json({
								status:500,
...
app.post('/signup',(req,res,next)=>{
	...
	User1.findOne({email_id:req.body.email},(err,user)=>{
		...
		return res.json({status:404 , message:"A user already exists with same email id"})
		else
		{
			next();
		}
...
function (req, res, next) {
  ...
  if (req.body.password !== req.body.passwordconf) {
    ...
    var hashedPassword = bcrypt.hashSync(req.body.password, 8);
        ...
        function (err, user) {
          if (err) return res.json({status:500,message:"There was a problem registering the user."})
					// create a token
					else
					{
...
app.post('/login',(req,res)=>{
	...
	console.log(req.connection.remoteAddress)
        ...
        // try{
				// if((user.ip).substr(0,10)!=req.connection.remoteAddress.substr(0,10))
				// {
				// 	return res.json({status:410 , message: 'Please connect to lan and use laptop' });
				// }
		   	// }
		  	// catch(err)
		   	// {
				// 	console.log("err in substr part");
				// 	return res.json({status:410 , message: 'Please connect to lan and use laptop' });
		  	...
		  	// }
				...
				if (!passwordIsValid) return res.json({status:401 , auth: false, token: null,message:'Incorrect password , try again' });
